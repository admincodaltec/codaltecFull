...
const formSchema = z.object({
	...
	esDescription: z
		...
		.max(150, {message: 'La descripción debeter menos de 150 caracteres'}),
	enDescription: z
		.string({message: 'El campo es requerido'})
		.min(4, {
			message: 'La descripción debe tener mas de 4 caracteres.',
	...
	async function onSubmit(values: z.infer<typeof formSchema>) {
		...
		try {
			...
			setTimeout(() => {
				setIsLoading(false);
				if (response.ok) {
					toast('👍 Slider registrado con exito!');
		...
		<>
			...
			<div className='flex-1 space-y-4 p-4 pt-6 md:p-8'>
				<div className='flex items-start justify-between'>
					<Heading title={`Crear slider`} description='Agrega un nuevo slider' />
				</div>
				<Separator />
				<Form {...form}>
					<form onSubmit={form.handleSubmit(onSubmit)} className='space-y-8'>
						<FormField
							...
							render={({field}) => (
								<FormItem>
									<FormLabel>Imagen del Slider</FormLabel>
									<FormControl>
										<Input type='file' placeholder='shadcn' {...fileRef} />
									</FormControl>
									<FormDescription>Imagen que aparecera de fondo del slider.</FormDescription>
									<FormMessage />
								</FormItem>
							)}
						/>
